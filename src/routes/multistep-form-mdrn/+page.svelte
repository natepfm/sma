<script lang="ts">
  ...
  // Affiliate tracking parameters - test defaults for PolicySideKick
  ...
  // Compliance tokens - captured early and stored for session
  let trustedFormCertUrl = $state<string>('');
  let jornayaLeadId = $state<string>('');
  ...
  if (typeof window !== 'undefined') {
    ...
    // Load stored affiliate params (will be overridden by URL params in onMount if present)
    const storedAffiliateParams = localStorage.getItem('savemaxauto_affiliate_params');
    if (storedAffiliateParams) {
      try {
        const parsed = JSON.parse(storedAffiliateParams);
        if (parsed.c) affiliateParams.c = parsed.c;
        if (parsed.source) affiliateParams.source = parsed.source;
        if (parsed.pcid) affiliateParams.pcid = parsed.pcid;
        if (parsed.pmclid) affiliateParams.pmclid = parsed.pmclid;
      } catch (e) {
        // Keep defaults
      }
    }

    // Load stored compliance tokens
    const storedTrustedForm = localStorage.getItem('savemaxauto_trustedform_cert');
    const storedJornaya = localStorage.getItem('savemaxauto_jornaya_id');
    if (storedTrustedForm) trustedFormCertUrl = storedTrustedForm;
    if (storedJornaya) jornayaLeadId = storedJornaya;
  ...
  async function createSession() {
    try {
      // Build refererUrl with affiliate tracking params for PolicySideKick
  ...
  async function autoCompleteZip(zip: string) {
    try {
      console.log('autoCompleteZip called with:', zip);
      const response = await fetch('/api/savemaxauto/proxy', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
      ...
      const result = await response.json();
      console.log('autoCompleteZip response:', result);
      
      if (result.success && result.data) {
        city = result.data.city || '';
        state = result.data.state || '';
        console.log('City/State set to:', city, state);
        // Store in localStorage
        localStorage.setItem('savemaxauto_city', city);
        localStorage.setItem('savemaxauto_state', state);
      } else {
        console.error('autoCompleteZip failed - no success or data:', result);
      }
  ...
  async function submitForm() {
    try {
      ...
      // Get compliance tokens - prefer stored values, fallback to DOM/window
      let finalTrustedFormCert = trustedFormCertUrl;
      let finalJornayaId = jornayaLeadId;
      
      // If not captured yet, try to get from DOM/window as fallback
      if (!finalTrustedFormCert) {
        const tfInput = document.getElementById('xxTrustedFormCertUrl') as HTMLInputElement;
        if (tfInput?.value) {
          finalTrustedFormCert = tfInput.value;
          trustedFormCertUrl = finalTrustedFormCert; // Store for future
          localStorage.setItem('savemaxauto_trustedform_cert', finalTrustedFormCert);
        }
      }
      
      if (!finalJornayaId) {
        const jornayaToken = (window as any).LeadiD?.token;
        if (jornayaToken) {
          finalJornayaId = jornayaToken;
          jornayaLeadId = finalJornayaId; // Store for future
          localStorage.setItem('savemaxauto_jornaya_id', finalJornayaId);
        }
      }
      
      console.log('TrustedForm cert URL (final):', finalTrustedFormCert);
      console.log('Jornaya ID (final):', finalJornayaId);
      ...
      const payload = {
        hint: "noRoot",
        jornayaId: finalJornayaId,
        trustedFormCertUrl: finalTrustedFormCert,
        customer: {
          firstName,
          lastName,
          email,
  ...
  onMount(() => {
    // Run async setup in IIFE (onMount can't be async when returning cleanup)
    (async () => {
    ...
    // Load stored city/state from localStorage (if not already set)
    const storedCity = localStorage.getItem('savemaxauto_city');
    const storedState = localStorage.getItem('savemaxauto_state');
    if (storedCity && !city) city = storedCity;
    if (storedState && !state) state = storedState;
    
    // Capture affiliate tracking params from URL (override defaults if present)
    const urlC = queryParams.get('c');
    const urlSource = queryParams.get('source');
    const urlPcid = queryParams.get('pcid');
    const urlPmclid = queryParams.get('pmclid');
    
    if (urlC) affiliateParams.c = urlC;
    if (urlSource) affiliateParams.source = urlSource;
    if (urlPcid) affiliateParams.pcid = urlPcid;
    if (urlPmclid) affiliateParams.pmclid = urlPmclid;
    
    // Store affiliate params in localStorage for persistence
    localStorage.setItem('savemaxauto_affiliate_params', JSON.stringify(affiliateParams));
    
    // Load TrustedForm script properly (not via {@html} which doesn't execute scripts)
    const trustedFormScript = document.createElement('script');
    trustedFormScript.type = 'text/javascript';
    trustedFormScript.async = true;
    trustedFormScript.src = `https://api.trustedform.com/trustedform.js?field=xxTrustedFormCertUrl&ping_field=xxTrustedFormPingUrl&l=${Date.now()}${Math.random()}`;
    document.head.appendChild(trustedFormScript);
    
    // Load Jornaya LeadiD script properly
    ...
    jornayaScript.src = 'https://create.lidstatic.com/campaign/e943bced-c610-90b4-52a5-e0d12d103fcb.js?snippet_version=2';
    document.head.appendChild(jornayaScript);
    
    // Capture compliance tokens early - poll until they're available
    let complianceCheckAttempts = 0;
    const maxComplianceAttempts = 30; // Try for 30 seconds
    const complianceInterval = setInterval(() => {
      complianceCheckAttempts++;
      
      // Check for TrustedForm cert URL
      if (!trustedFormCertUrl) {
        const tfInput = document.getElementById('xxTrustedFormCertUrl') as HTMLInputElement;
        if (tfInput?.value) {
          trustedFormCertUrl = tfInput.value;
          localStorage.setItem('savemaxauto_trustedform_cert', trustedFormCertUrl);
          console.log('TrustedForm cert captured early:', trustedFormCertUrl);
        }
      }
      ...
      if (!jornayaLeadId) {
        const jornayaToken = (window as any).LeadiD?.token;
        if (jornayaToken) {
          jornayaLeadId = jornayaToken;
          localStorage.setItem('savemaxauto_jornaya_id', jornayaLeadId);
          console.log('Jornaya ID captured early:', jornayaLeadId);
        }
      }
      
      // Stop polling if both are captured or max attempts reached
      if ((trustedFormCertUrl && jornayaLeadId) || complianceCheckAttempts >= maxComplianceAttempts) {
        clearInterval(complianceInterval);
        if (complianceCheckAttempts >= maxComplianceAttempts) {
          console.warn('Compliance token capture timed out. TrustedForm:', !!trustedFormCertUrl, 'Jornaya:', !!jornayaLeadId);
        }
      }
